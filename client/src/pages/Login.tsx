...
const Login = () => {
  ...
  const handleQuickLogin = async () => {
    // DEBUG: Quick login handler - version 2
    if (phone.length < 8) {
      showError("Please enter a valid mobile number");
      return;
    }
            ...
            {step === 1 ? (
              ...
              >
                ...
                <Button 
                  ...
                  disabled={isLoading}
                  variant="outline"
                  className="w-full h-16 border-sky-500 text-sky-500 font-black rounded-3xl hover:bg-sky-50 transition-all group active:scale-95"
                >
                  {isLoading ? "LOGGING IN..." : "INSTANT LOGIN"}
                  {!isLoading && <ArrowRight className="ml-2 h-5 w-5 group-hover:translate-x-1 transition-transform" />}
                </Button>
              </motion.div>
            ) : (
