...
const ProfileNavigation = () => {
  ...
  return (
    <div className="glass-panel p-2 rounded-[2rem] border-slate-200 sticky top-24 z-30 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-1">
        {sections.map((item) => (
          <button 
            key={item.name} 
            onClick={() => scrollToSection(item.target)}
            className="flex-1 min-w-[140px] flex items-center justify-center gap-3 py-4 px-6 rounded-2xl transition-all duration-300 group text-[#0B1F3A]/60 hover:bg-sky-50 hover:text-sky-600"
          >
