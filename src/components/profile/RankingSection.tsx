...
import { Search, Trophy, TrendingUp, TrendingDown, Minus, Globe, MapPin, Building, Flag } from 'lucide-react';
...
const RankingSection = () => {
  ...
  const [scope, setScope] = useState("world"); // world, country, state, regional
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h3 className="text-xl font-black text-[#0B1F3A] flex items-center gap-3">
            <Trophy className="h-6 w-6 text-sky-500" /> Ranking Context
          </h3>
          ...
          {scopes.map((s) => (
            <button
              ...
              className={cn(
                ...
                scope === s.id ? "bg-white text-sky-600 shadow-sm" : "text-slate-400 hover:text-[#0B1F3A]"
              )}
            >
