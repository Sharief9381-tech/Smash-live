...
const Rankings = () => {
  ...
  const filteredRankings = useMemo(() => {
    ...
    if (userProfile) {
      ...
      if (!userExists) {
        list.push({ id: 999, rank: 1, name: userProfile.name, country: userProfile.country, state: userProfile.state, points: 105400, change: "up", diff: 1, matches: 842, winRate: "88", smashAcc: "94", img: userProfile.name.split(' ').map((n: any) => n[0]).join(''), isUser: true });
      }
    }
      ...
      <main className="container px-6 py-16 space-y-16">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
          ...
          <div className="flex items-center gap-2 bg-slate-100 p-1.5 rounded-2xl">
            {scopes.map((s) => (
              <button key={s.id} onClick={() => setScope(s.id)} className={cn("px-6 py-3 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all", scope === s.id ? "bg-[#0B1F3A] text-white shadow-lg" : "text-slate-400")}>{s.label}</button>
            ))}
          </div>
        </div>
          ...
          <Table>
            ...
            <TableBody>
              {filteredRankings.length > 0 ? filteredRankings.map((row, idx) => (
                <TableRow key={row.id} className={cn("h-24", row.isUser && "bg-sky-50/30")}>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="font-black text-[#0B1F3A] text-base">{row.name} {row.isUser && <span className="ml-1 text-[8px] bg-sky-500 text-white px-1.5 py-0.5 rounded-full">YOU</span>}</h4>
                  ...
                  <TableCell className="text-right pr-12">
                    <div className="flex items-center justify-end gap-1">
                      {row.change === 'up' && <div className="flex items-center text-green-500 font-black"><ChevronUp className="h-4 w-4" /> <span className="text-xs">{row.diff || 1}</span></div>}
                      {row.change === 'down' && <div className="flex items-center text-red-500 font-black"><ChevronDown className="h-4 w-4" /> <span className="text-xs">{row.diff || 1}</span></div>}
